...
for index_train in range(train_size):
	...
	train_lbls[index_train] = np.transpose( np.reshape(np.array(train_labels[index_train]), newshape=(dim,1) ) )
...
for index_valid in range(valid_size):
	...
	valid_lbls[index_valid] = np.transpose( np.reshape(np.array(valid_labels[index_valid]), newshape=(dim,1) ) )
...
nn_instance.addHiddenLayer(2000, 1, activation_fn=tf.nn.sigmoid)
#nn_instance.addHiddenLayer(1500, 2, activation_fn=tf.nn.relu)
#nn_instance.addHiddenLayer(1000, 3, activation_fn=tf.nn.relu)
...
with tf.Session() as sess:
	...
	test_pred = nn_instance.train(sess, max_epochs, batch_size, train_size, train_signal, train_lbls, valid_signal, valid_lbls, test_signal) # add more arguments here
	np.savetxt("predictions_speech.txt", test_pred[0], fmt='%d')
	print(test_pred[0])
	print("Output saved in the generated file")